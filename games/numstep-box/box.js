...
const CELL_ADJACENCY=Object.freeze({
    ...
    10: [11, 13, 27, 34], 11: [10, 12, 14, 35], 12: [11, 15, 36, 43],
...
async function shareResult(){if(typeof showShareModal!=="function")return;const elapsed=startedAt?Date.now()-startedAt:0,seconds=Math.floor(elapsed/1000),formattedTime=`${String(Math.floor(seconds/60)).padStart(2,"0")}:${String(seconds%60).padStart(2,"0")}`,badgeImageUrl=typeof NumstepBadge!=="undefined"?await NumstepBadge.generate(puzzle.size,fd(selectedDate),formattedTime,attempts,"box"):"";showShareModal({size:puzzle.size,date:fd(selectedDate),elapsed,attempts,url:window.location.href,badgeImageUrl})}
