 ...
 *   - Loads: data/[size]x[size]/[YYYY-MM-DD].json
...
const PUZZLE_DATA_PATH = "data";
const TAURUS_MODE = true;
...
function getPuzzleFilename(size) {
    ...
    return `${PUZZLE_DATA_PATH}/${size}x${size}/${selectedDate}.json`;
}
...
function isReachable(start, target, ignoreChainClue = null) {
    ...
    while (queue.length > 0) {
        ...
        for (const direction of DIRECTIONS) {
            const nr = (current.r + direction.r + puzzleSize) % puzzleSize;
            const nc = (current.c + direction.c + puzzleSize) % puzzleSize;
            const key = `${nr},${nc}`;
...
function isAdjacent(
    ...
    if (TAURUS_MODE) {
        return (
            (columnDifference === 0 && (rowDifference === 1 || rowDifference === puzzleSize - 1)) ||
            (rowDifference === 0 && (columnDifference === 1 || columnDifference === puzzleSize - 1))
        );
    }
    return (
