...
def torus_neighbours(row, col, n):
    """Orthogonal neighbours on a torus: opposite edges wrap together."""
    return [((row - 1) % n, col), ((row + 1) % n, col), (row, (col - 1) % n), (row, (col + 1) % n)]
...
def count_solutions(grid, max_solutions=2):
    ...
    for row, col in white_cells:
        ...
        for new_row, new_col in torus_neighbours(row, col, n):
            if grid[new_row][new_col] != 0:
                cell_neighbours.append((new_row, new_col))
...
def generate_walk(n, threshold=0.80, require_unique=True):
    ...
    while True:
        ...
        if not require_unique:
            print(f"  Valid Taurus share puzzle found after {attempts} attempts (uniqueness not required).")
            return grid, steps
...
for size in sizes:
    ...
    web_filename = (
        Path("games") / "numstep-taurus" / "data" / f"{size}x{size}" /
...
filename = (
    Path("games") / "numstep-taurus" / "printables" /
